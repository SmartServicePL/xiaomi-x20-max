...
class X20MaxController:
    ...
    async def async_initialize(self) -> None:
        ...
        if self.entry.data.get("model", MODEL) != MODEL:
            raise X20MaxNotReady(f"This integration supports only model {MODEL}")
        try:
    ...
    async def async_refresh(self) -> None:
        ...
        async with self._refresh_lock:
            ...
            if not values:
                raise X20MaxError(f"{self.name}: Xiaomi Cloud returned no device data")
    ...
    def available(self) -> bool:
        return bool(
            self._last_success
            and time_module.monotonic() - self._last_success < AVAILABLE_GRACE_SECONDS
        )
    ...
    async def async_set_property(self, ref: PropertyRef, value: Any) -> None:
        try:
            ...
            raise X20MaxError(
                f"Could not set MIoT property {ref.siid}.{ref.piid}: {err}"
            ) from err
    ...
    async def async_action(
        ...
        try:
            ...
            raise X20MaxError(
                f"Could not execute MIoT action {ref.siid}.{ref.aiid}: {err}"
            ) from err
        self._schedule_delayed_refresh()
        ...
        for value in values:
            if isinstance(value, int) or str(value).strip().isdigit():
                ...
                if known_ids and room_id not in known_ids:
                    raise X20MaxError(f"Unknown room ID: {room_id}")
            else:
                key = str(value).strip().casefold()
                if key not in by_name:
                    raise X20MaxError(f"Unknown room name: {value}")
                room_id = by_name[key]
            if room_id not in resolved:
                resolved.append(room_id)
        if not resolved:
            raise X20MaxError("At least one room is required")
        return resolved
    ...
    async def async_set_dnd(self, start: time, end: time, enabled: bool = True) -> None:
        packed = (
            (start.hour << 24) | (start.minute << 16) | (end.hour << 8) | end.minute
        )
